...
const Canvas: React.FC<CanvasProps> = ({ config, globalSettings }) => {
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    // Adjust scale based on drag distance (drag up to scale up, drag down to scale down)
    scaleRef.current = Math.max(0.2, Math.min(3.0, scaleRef.current - deltaY * 0.01));
  };
  ...
  const handleWheel = (e: React.WheelEvent<HTMLCanvasElement>) => {
    // Prevent default scrolling when hovering over canvas
    scaleRef.current = Math.max(0.2, Math.min(3.0, scaleRef.current - e.deltaY * 0.001));
  };
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      balls.push({
        ...
        color: config.colors && config.colors.length > 0 
  ...
  const updatePhysics = (width: number, height: number) => {
    ...
    const shapeRadius = Math.min(width, height) * 0.45 * scaleRef.current; // 45% of canvas size scaled
  ...
  const draw = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    const center = { x: width / 2, y: height / 2 };
    const state = stateRef.current;
    const shapeRadius = Math.min(width, height) * 0.45 * scaleRef.current;
  ...
  return (
    <canvas 
      ref={canvasRef} 
      className="w-full h-full block touch-none cursor-ns-resize" 
